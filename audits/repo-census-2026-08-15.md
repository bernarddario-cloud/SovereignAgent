...
5. **Medium — duplicated live n8n hooks can drift independently.** `server/routes.ts:178-210` (voice) and `server/routes.ts:232-264` (chat) are 33-line, 93.9%-similar implementations that construct and submit their own event payloads. Symptom: a repair to telemetry, authorization, or webhook handling in one path can leave the other sending divergent downstream action data.
...
- **Byte-identical, 45 lines:** `server/routes/parliament.ts:1-45` and `.ts:1-45`.
  ...
  - Nested copy: no runtime import enters `server/intelligence/server/`; its relative imports do not resolve at that depth. It is a **git-verified stale orphan**, created in `d5c8378` and never subsequently modified.
- **Near-duplicate, 33 lines / 93.9% similar:** `server/routes.ts:178-210` (voice n8n hook) and `server/routes.ts:232-264` (text n8n hook). Both are active in their respective request paths. The action name and input shape differ; the webhook control flow is duplicated. Stale copy: **cannot determine**, because both are live implementations in the same file.
- **Byte-identical aggregation core, 10 lines:** `server/intelligence/parliament.ts:14-23` and `.ts:19-28`. The surrounding implementations diverge: the active canonical version uses four stub minds at `server/intelligence/minds.ts:4-41`; the nested version uses richer minds/compliance code. The nested copy is an orphaned/stale implementation; whether it was intended to replace the stubs is **not determinable**.
- **Near-duplicate mind templates, 17 lines:** nested `.ts:4-20` and `.ts:4-20`. These are stale/orphaned along with the nested tree; the duplication is governance scoring, not a verified live order path.
- **Parallel ledger implementations (conceptual duplication, not a byte-identical 15-line block):** `server/services/ledger.ts:50-140` (audit ledger used by consent/OAuth/token routes), `server/ledger/storage.ts:1-17` plus `server/routes/ledger.ts:1-11` (in-memory records endpoint), and nested `.ts:1-31` (file-backed intent ledger). The latter nested implementation is stale/unreachable; the two canonical ledger systems are both wired to different endpoints, so which is the intended single authority is **not determinable** from the code.

## 3. CLONE MAP

| Canonical/active path | Copy path | Evidence | Staleness |
| --- | --- | --- | --- |
| `server/routes/parliament.ts:1-45` | `.ts:1-45` | Identical SHA-256 `caffeda90bf80ae09b698ca9ecec25a4776a96afebdc0cf7df03c189a620b192`; active import/mount at `server/routes.ts:23,73` | Nested copy is a stale orphan: no runtime import enters the tree; broken relative imports; created only in `d5c8378`. |
| `server/routes.ts:178-210` | `server/routes.ts:232-264` | 33-line n8n blocks, 93.9% similar; both submit an event through `postToN8N` | Neither is stale; both paths are live. |
| `server/intelligence/parliament.ts:14-23` | `.ts:19-28` | Exact vote-aggregation core; canonical uses stubs, nested uses richer governance | Nested file is stale/unreachable; intended replacement direction cannot be determined. |
| `server/services/ledger.ts:50-140` | `server/ledger/storage.ts:1-17`; nested `.ts:1-31` | Parallel ledger designs and overlapping records/intent responsibilities | Nested design is stale/unreachable; canonical dual-ledger authority cannot be determined. |

No duplicated block touching market prices, orders, or position sizing was found because no such implementation was found in the tracked snapshot. The n8n hooks and parallel ledger designs do touch action telemetry/audit records and are the highest-risk duplication found.
...
- Whether the nested rich governance/mind implementation was intended to supersede the active stub implementation: git history verifies that it is orphaned and stale, but does not establish the intended architecture.
- Which of the two wired canonical ledger systems (`server/services/ledger.ts` and `server/ledger/storage.ts`) is the single intended audit authority: they serve distinct endpoints and the code does not declare one canonical.
